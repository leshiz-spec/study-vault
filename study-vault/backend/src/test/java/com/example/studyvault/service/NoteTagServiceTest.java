...
class NoteTagServiceTest {
    private NoteRepository notes; private NoteTagRepository noteTags; private TagService tags; private NoteTagService service;
    private User alice; private User bob; private Note note; private Tag tag;

    @BeforeEach
    void setUp() {
        notes = mock(NoteRepository.class); noteTags = mock(NoteTagRepository.class); tags = mock(TagService.class);
        service = new NoteTagService(notes, tags, noteTags);
        alice = user(1L); bob = user(2L); note = note(10L, alice); tag = tag(20L, alice);
    }
    ...
    private static User user(Long id) { User u = new User(); setId(u, id); return u; }
    private static Note note(Long id, User user) { Note n = new Note(); setId(n, id); n.setUser(user); n.setTitle("Note"); n.setContent("Body"); return n; }
    private static Tag tag(Long id, User user) { Tag t = new Tag(); setId(t, id); t.setUser(user); t.setName("java"); return t; }
