...
class AuthControllerTest {
    private MockMvc mvc; private AuthService auth;
    @BeforeEach void setUp() { auth = mock(AuthService.class); mvc = MockMvcBuilders.standaloneSetup(new AuthController(auth)).setControllerAdvice(new GlobalExceptionHandler()).build(); }

    @Test void registrationReturnsHttpOnlyCookieAndSafeUser() throws Exception {
        when(auth.register(any())).thenReturn(new AuthService.AuthResult(publicResponse(), "jwt-token"));
        mvc.perform(post("/api/auth/register").contentType("application/json").content("{\"username\":\"alice\",\"email\":\"a@example.com\",\"password\":\"secret123\"}"))
                .andExpect(status().isOk()).andExpect(header().string("Set-Cookie", containsString("HttpOnly"))).andExpect(jsonPath("$.data.username").value("alice")).andExpect(content().string(not(containsString("password_hash"))));
    }

    @Test void unauthenticatedMeReturns401() throws Exception { mvc.perform(get("/api/auth/me")).andExpect(status().isUnauthorized()).andExpect(jsonPath("$.error.code").value("UNAUTHORIZED")); }
    ...
    private UserResponse publicResponse() { return new UserResponse(null, "alice", "a@example.com", null, null); }
}
