...
class StudyTaskControllerTest {
    ...
    void setUp() {
        ...
        mvc = MockMvcBuilders.standaloneSetup(new StudyTaskController(service))
    ...
    void createUpdateAndDeleteTask() throws Exception {
        when(service.create(eq(alice), any(StudyTaskCreateRequest.class))).thenReturn(task(1L, "Review", "todo"));
        when(service.update(eq(alice), eq(1L), any(StudyTaskUpdateRequest.class))).thenReturn(task(1L, "Review", "done"));

        mvc.perform(post("/api/tasks").principal(auth(alice)).contentType("application/json")
                        .content("{\"title\":\"Review\",\"status\":\"todo\",\"noteIds\":[3,4]}"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.id").value(1));
        verify(service).create(eq(alice), argThat(request -> request.noteIds().equals(List.of(3L, 4L))));
        mvc.perform(put("/api/tasks/1").principal(auth(alice)).contentType("application/json")
    ...
    private static StudyTaskResponse task(Long id, String title, String status) { return new StudyTaskResponse(id, title, null, status, null, null, null, null); }
}
