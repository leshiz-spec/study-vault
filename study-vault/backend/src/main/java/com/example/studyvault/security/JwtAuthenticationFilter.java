...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    @Override protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        if (token != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try { Long userId = Long.valueOf(jwtService.parse(token).getSubject()); users.findById(userId).ifPresent(user -> {
            ...
            }); } catch (JwtException | IllegalArgumentException ignored) { /* invalid tokens are treated as anonymous */ }
        }
        chain.doFilter(request, response);
    }
    private String cookie(HttpServletRequest request) { if (request.getCookies() == null) return null; for (Cookie c : request.getCookies()) if (COOKIE_NAME.equals(c.getName())) return c.getValue(); return null; }
}
