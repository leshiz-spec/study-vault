...
public class NoteFileService {
    ...
    public NoteResponse importMarkdown(User user, MultipartFile file) {
        ...
        if (bytes.length > MAX_IMPORT_BYTES) throw new FileTooLargeException("Markdown file must be at most 5 MB");
    ...
    static String safeFilename(String title, Long id) {
        ...
        normalized = normalized.replaceAll("[\\\\/\\p{Cntrl}]+", " ")
                ...
                .replaceAll("\\s+", " ").trim()
                .replaceAll("^[. ]+|[. ]+$", "");
        if (normalized.isBlank() || normalized.equals(".") || normalized.equals("..")) normalized = "note";
    ...
    private Note owned(User user, Long id) {
        return notes.findByIdAndUser(id, user).orElseThrow(() -> new com.example.studyvault.exception.NoteNotFoundException(id));
    }
    ...
    private void validateFile(MultipartFile file) {
        if (file == null || file.isEmpty()) throw new InvalidMarkdownFileException("A non-empty Markdown file is required");
        if (file.getSize() > MAX_IMPORT_BYTES) throw new FileTooLargeException("Markdown file must be at most 5 MB");
    ...
    private ParsedMarkdown parse(String source, String filename) {
        ...
        if (title.length() > 255) throw new InvalidMarkdownFileException("The Markdown title must be at most 255 characters");
        ...
        if (content.isBlank()) throw new InvalidMarkdownFileException("The Markdown file must contain note content");
        if (content.length() > 1_000_000) throw new InvalidMarkdownFileException("Note content must be at most 1000000 characters");
