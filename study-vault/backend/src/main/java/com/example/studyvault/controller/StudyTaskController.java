...
public class StudyTaskController {
    ...
    public StudyTaskController(StudyTaskService tasks) { this.tasks = tasks; }
    ...
    public ApiResponse<List<StudyTaskResponse>> list(
        ...
        return ApiResponse.success(tasks.list(user(authentication), status, dueDate, dueAfter, dueBefore));
    }

    @PostMapping
    public ApiResponse<StudyTaskResponse> create(Authentication authentication, @Valid @RequestBody StudyTaskCreateRequest request) {
        return ApiResponse.success(tasks.create(user(authentication), request));
    }

    @PutMapping("/{id}")
    public ApiResponse<StudyTaskResponse> update(Authentication authentication, @PathVariable Long id,
