...
class NoteSearchRepositoryTest {
  ...
  @Autowired
  NoteSearchRepositoryTest(
      NoteRepository notes, UserRepository users, TagRepository tags, NoteTagRepository noteTags) {
    this.notes = notes;
    this.users = users;
    this.tags = tags;
  ...
  void combinesQueryTagFavoriteStatusPagingAndSort() {
    var page =
        notes.findAll(
            NoteSpecifications.search(alice, "needle", "math", true, "active"),
  ...
  void ownershipPredicateExcludesAnotherUsersMatchingNote() {
    var page =
        notes.findAll(
            NoteSpecifications.search(alice, "needle", null, null, "active"),
            PageRequest.of(0, 20));

    assertEquals(1, page.getTotalElements());
    org.junit.jupiter.api.Assertions.assertTrue(
        page.getContent().stream().noneMatch(note -> note.getUser() != alice));
  }
  ...
  private static Note note(
      User user, String title, String content, String status, boolean favorite) {
    Note note = new Note();
    note.setUser(user);
    note.setTitle(title);
