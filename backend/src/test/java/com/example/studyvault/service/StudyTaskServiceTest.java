...
class StudyTaskServiceTest {
    ...
    void createTaskScopesOwnerAndCanLinkOwnedNote() {
        ...
        when(tasks.save(any(StudyTask.class))).thenAnswer(invocation -> {
            StudyTask task = invocation.getArgument(0);
            setId(task, 5L);
            return task;
        });

        var result = service.create(alice, new StudyTaskCreateRequest("Review graphs", LocalDate.of(2026, 9, 10), "todo", 10L));

        assertEquals(5L, result.id());
        assertEquals(10L, result.noteId());
        verify(tasks).save(argThat(task -> task.getUser() == alice && task.getNote() == note
    ...
    void createTaskCanLinkMultipleOwnedNotesWithoutDuplicates() {
        ...
        var result = service.create(alice,
                new StudyTaskCreateRequest("Review topics", null, "todo", null, List.of(10L, 11L, 10L)));

        assertEquals(List.of(10L, 11L), result.noteIds());
        assertEquals(List.of("Algorithms", "Databases"), result.noteTitles());
        verify(tasks).save(argThat(task -> task.getNotes().size() == 2
                && task.getNotes().containsAll(List.of(note, secondNote))));
    }

    @Test
    void listUsesAuthenticatedOwnerAndFilters() {
        when(tasks.findForUser(alice, "in_progress", LocalDate.of(2026, 9, 10), null, null)).thenReturn(List.of());
    ...
    void updateTaskAndDeleteOnlyUseOwnedTask() {
        ...
        var updated = service.update(alice, 5L,
    ...
    void linkingMultipleNotesFailsIfAnyNoteIsNotOwned() {
        ...
        assertThrows(NoteNotFoundException.class,
                () -> service.create(alice,
                        new StudyTaskCreateRequest("Task", null, null, null, List.of(10L, 99L))));
    ...
    private static User user(Long id, String username) { User user = new User(); setId(user, id); user.setUsername(username); return user; }
    private static Note note(Long id, User user, String title) { Note note = new Note(); setId(note, id); note.setUser(user); note.setTitle(title); note.setContent("Body"); return note; }
    private static StudyTask task(Long id, User user, String title) { StudyTask task = new StudyTask(); setId(task, id); task.setUser(user); task.setTitle(title); task.setStatus("todo"); return task; }
