...
class NoteRevisionServiceTest {
    ...
    void listUsesNoteAndUserOwnershipQuery() {
        ...
        when(revisions.findAllByNote_IdAndNote_User_IdOrderByCreatedAtDesc(10L, 1L)).thenReturn(List.of(revision));
    ...
    void restoreDelegatesToNoteUpdateSoCurrentStateBecomesNewRevision() {
        Note note = note(10L, alice, "Current", "Current body");
        NoteRevision revision = revision(20L, note, "Old", "Old body");
        NoteResponse restored = new NoteResponse(10L, "Old", "Old body", null, "active", false, "not_started", null, null);
        when(notes.findByIdAndUser(10L, alice)).thenReturn(Optional.of(note));
        when(revisions.findByIdAndNote_IdAndNote_User_Id(20L, 10L, 1L)).thenReturn(Optional.of(revision));
        ...
        verify(noteService).update(eq(alice), eq(10L), argThat(request -> request.title().equals("Old") && request.content().equals("Old body")));
    }

    private static User user(Long id, String username) { User user = new User(); setId(user, id); user.setUsername(username); return user; }
    private static Note note(Long id, User user, String title, String content) { Note note = new Note(); setId(note, id); note.setUser(user); note.setTitle(title); note.setContent(content); return note; }
    private static NoteRevision revision(Long id, Note note, String title, String content) { NoteRevision revision = new NoteRevision(); setId(revision, id); revision.setNote(note); revision.setTitle(title); revision.setContent(content); return revision; }
