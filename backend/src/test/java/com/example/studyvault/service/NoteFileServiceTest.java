...
class NoteFileServiceTest {
  ...
  void zipExportContainsOnlyOwnedNotes() throws Exception {
    User bob = new User();
    bob.setUsername("bob");
    when(notes.findAllByUserOrderByUpdatedAtDesc(alice))
        .thenReturn(
            List.of(
                note(1L, alice, "First", "one"),
                note(2L, bob, "Other user", "private"),
  ...
  void importMarkdownCreatesOwnedNoteFromHeading() {
    ...
    when(noteService.create(eq(alice), any()))
        .thenReturn(
            new NoteResponse(
                3L, "Imported", "Lesson body", null, "active", false, "not_started", null, null));
    ...
    verify(noteService)
        .create(
            ...
            argThat(
                request ->
                    request.title().equals("Imported") && request.content().equals("Lesson body")));
  }
