...
class AuthServiceTest {
  ...
  @Test
  void successfulRegistrationHashesPasswordAndReturnsToken() {
    when(users.existsByEmail("a@example.com")).thenReturn(false);
    when(users.existsByUsername("alice")).thenReturn(false);
    when(encoder.encode("secret123")).thenReturn("$2a$hashed");
    when(users.save(any()))
        .thenAnswer(
            inv -> {
    ...
    when(jwt.issue(null, "alice")).thenReturn("jwt");
  ...
  @Test
  void duplicateEmailUsesStableError() {
    ...
    assertEquals("EMAIL_ALREADY_EXISTS", ex.getCode().name());
    verify(users, never()).save(any());
  }

  @Test
  void duplicateUsernameUsesStableErrorEvenWhenEmailAlsoExists() {
  ...
  @Test
  void successfulLoginChecksPassword() {
    User user = user();
    when(users.findByUsername("alice")).thenReturn(java.util.Optional.of(user));
    when(encoder.matches("secret123", "hash")).thenReturn(true);
    when(jwt.issue(7L, "alice")).thenReturn("jwt");
    var result = service.login(new LoginRequest("alice", "secret123"));
    assertEquals("jwt", result.token());
    verify(encoder).matches("secret123", "hash");
  }
  ...
  void invalidPasswordIsUnauthorized() {
    ...
    var ex =
        assertThrows(
            UnauthorizedException.class, () -> service.login(new LoginRequest("alice", "wrong")));
