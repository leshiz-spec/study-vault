...
class SearchControllerTest {
  ...
  void searchPassesAllFiltersAndPagingToService() throws Exception {
    when(service.search(
            eq(alice), eq("graph"), eq("math"), eq(true), eq("active"), any(Pageable.class)))
    ...
    verify(service)
        .search(eq(alice), eq("graph"), eq("math"), eq(true), eq("active"), any(Pageable.class));
  }
  ...
  private static NoteResponse response() {
    return new NoteResponse(
        1L, "Algorithms", "Graph search", null, "active", true, "not_started", null, null);
  }
}
