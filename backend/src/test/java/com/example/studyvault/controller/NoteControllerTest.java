...
class NoteControllerTest {
  ...
  void setUp() {
    ...
    noteTags = mock(NoteTagService.class);
    summaries = mock(NoteSummaryService.class);
    revisions = mock(NoteRevisionService.class);
    mvc =
        MockMvcBuilders.standaloneSetup(
                new NoteController(service, noteTags, null, summaries, revisions))
  ...
  void createNote() throws Exception {
    when(service.create(eq(alice), any(NoteCreateRequest.class)))
        .thenReturn(response(1L, "Title", "Body", "active"));
    mvc.perform(
            post("/api/notes")
    ...
    verify(service)
        .create(
            eq(alice),
            argThat(
                request -> request.title().equals("Title") && request.content().equals("Body")));
  }

  @Test
  void listOwnNotes() throws Exception {
    when(service.list(alice)).thenReturn(List.of(response(1L, "Mine", "Body", "active")));
    mvc.perform(get("/api/notes").principal(auth(alice)))
        .andExpect(status().isOk())
  ...
  void updateOwnNote() throws Exception {
    when(service.update(eq(alice), eq(1L), any(NoteUpdateRequest.class)))
        .thenReturn(response(1L, "New", "Body", "active"));
    mvc.perform(
            put("/api/notes/1")
                .principal(auth(alice))
                .contentType("application/json")
                .content("{\"title\":\"New\",\"content\":\"Body\"}"))
        .andExpect(status().isOk())
  ...
  void updateReviewStatus() throws Exception {
    when(service.updateReviewStatus(
            eq(alice), eq(1L), any(com.example.studyvault.dto.ReviewStatusRequest.class)))
        .thenReturn(
            new NoteResponse(1L, "Title", "Body", null, "active", false, "mastered", null, null));
    mvc.perform(
            put("/api/notes/1/review-status")
                .principal(auth(alice))
                .contentType("application/json")
                .content("{\"reviewStatus\":\"mastered\"}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.reviewStatus").value("mastered"));
  }

  @Test
  void listNoteRevisions() throws Exception {
    when(revisions.list(alice, 1L))
        .thenReturn(List.of(new NoteRevisionResponse(2L, 1L, "Old", "Old body", null)));
  ...
  void deleteOwnNote() throws Exception {
    mvc.perform(delete("/api/notes/1").principal(auth(alice)))
        .andExpect(status().isOk())
  ...
  void saveSummaryRequiresExplicitRequestAndReturnsUpdatedNote() throws Exception {
    when(summaries.save(alice, 1L, "Draft summary"))
        .thenReturn(responseWithSummary(1L, "Title", "Body", "Draft summary"));
    mvc.perform(
            put("/api/notes/1/summary")
                .principal(auth(alice))
                .contentType("application/json")
  ...
  void nonexistentNoteReturnsNoteNotFound() throws Exception {
    ...
    mvc.perform(get("/api/notes/404").principal(auth(alice)))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.error.code").value("NOTE_NOT_FOUND"));
  }
  ...
  void anotherUsersNoteIsAlsoNotFound() throws Exception {
    ...
    mvc.perform(get("/api/notes/2").principal(auth(alice)))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.error.code").value("NOTE_NOT_FOUND"));
  }
  ...
  private static NoteResponse response(Long id, String title, String content, String status) {
    return new NoteResponse(id, title, content, null, status, false, "not_started", null, null);
  }

  private static NoteResponse response(
      Long id, String title, String content, String status, boolean favorite) {
    return new NoteResponse(id, title, content, null, status, favorite, "not_started", null, null);
  }

  private static NoteResponse responseWithSummary(
      Long id, String title, String content, String summary) {
    return new NoteResponse(
        id, title, content, summary, "active", false, "not_started", null, null);
  }
}
