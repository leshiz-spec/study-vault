...
class NoteFileControllerTest {
  ...
  void setUp() {
    ...
    mvc =
        MockMvcBuilders.standaloneSetup(
                new NoteController(mock(NoteService.class), mock(NoteTagService.class), files))
  ...
  void singleExportSetsMarkdownDownloadHeaders() throws Exception {
    ...
    doAnswer(
            invocation -> {
              ((OutputStream) invocation.getArgument(2)).write("# My\n".getBytes());
              return null;
            })
        .when(files)
        .writeMarkdown(eq(alice), eq(7L), any(OutputStream.class));
    mvc.perform(get("/api/notes/7/export").principal(auth(alice)))
        .andExpect(status().isOk())
        .andExpect(header().string("Content-Type", "text/markdown"))
        .andExpect(
            header().string("Content-Disposition", org.hamcrest.Matchers.containsString("My-7.md")))
  ...
  void zipExportSetsArchiveHeadersAndStreamsArchive() throws Exception {
    doAnswer(
            invocation -> {
              ((OutputStream) invocation.getArgument(1))
                  .write("zip-bytes".getBytes(StandardCharsets.UTF_8));
              return null;
            })
        .when(files)
        .writeZip(eq(alice), any(OutputStream.class));
    ...
    assertTrue(
        response.getHeaders().getFirst("Content-Disposition").contains("studyvault-notes.zip"));
  ...
  void importEndpointAcceptsMarkdownMultipartAndReturnsCreatedNote() throws Exception {
    when(files.importMarkdown(eq(alice), any(MockMultipartFile.class)))
        .thenReturn(
            new NoteResponse(
                3L, "Imported", "Body", null, "active", false, "not_started", null, null));
  ...
  void importEndpointReturnsStableErrorForInvalidUpload() throws Exception {
    when(files.importMarkdown(eq(alice), any(MockMultipartFile.class)))
        .thenThrow(
            new com.example.studyvault.exception.InvalidMarkdownFileException(
                "Only .md Markdown files are supported"));
    MockMultipartFile upload =
        new MockMultipartFile(
            "file", "lesson.txt", "text/plain", "Body".getBytes(StandardCharsets.UTF_8));
