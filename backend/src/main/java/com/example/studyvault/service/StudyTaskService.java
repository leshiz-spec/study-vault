...
public class StudyTaskService {
    ...
    public List<StudyTaskResponse> list(User user, String status, LocalDate dueDate, LocalDate dueAfter, LocalDate dueBefore) {
        validateStatus(status);
        return tasks.findForUser(user, status, dueDate, dueAfter, dueBefore).stream().map(StudyTaskResponse::from).toList();
    }
    ...
    public StudyTaskResponse create(User user, StudyTaskCreateRequest request) {
        ...
        task.setUser(user);
        apply(task, user, request.title(), request.dueDate(), status, request.noteIds(), request.noteId());
    ...
    public StudyTaskResponse update(User user, Long id, StudyTaskUpdateRequest request) {
        ...
        String status = request.status() == null || request.status().isBlank() ? task.getStatus() : request.status();
        validateStatus(status);
        apply(task, user, request.title(), request.dueDate(), status, request.noteIds(), request.noteId());
        return StudyTaskResponse.from(tasks.save(task));
    }
    ...
    private void apply(StudyTask task, User user, String title, LocalDate dueDate, String status,
                       List<Long> noteIds, Long legacyNoteId) {
        task.setTitle(title);
    ...
    private void validateStatus(String status) {
        if (status != null && !TASK_STATUSES.contains(status)) throw new InvalidTaskStatusException(status);
    }
}
