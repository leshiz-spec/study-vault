...
public class OpenAiSummaryProvider implements AiSummaryProvider {
    ...
    public OpenAiSummaryProvider(
        ...
        this.mapper = mapper;
        this.client = HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(Math.max(1, timeoutSeconds))).build();
    ...
    public String summarize(String noteContent) {
        ...
        try {
            requestJson = mapper.writeValueAsString(Map.of(
                    ...
                    "temperature", 0.2,
                    "messages", new Object[] {
                            Map.of("role", "system", "content", "Summarize the user's note clearly and briefly. Return only the summary."),
        ...
        if (response.statusCode() == 408 || response.statusCode() == 504) throw new AiTimeoutException();
        ...
        if (response.statusCode() < 200 || response.statusCode() >= 300) throw new AiProviderException();
        try {
            JsonNode content = mapper.readTree(response.body()).path("choices").path(0).path("message").path("content");
            if (!content.isTextual() || content.asText().isBlank()) throw new AiProviderException("The AI service returned an empty summary");
