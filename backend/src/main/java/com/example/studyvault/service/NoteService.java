...
public class NoteService {
  private static final Set<String> REVIEW_STATUSES =
      Set.of("not_started", "learning", "review", "mastered");
  ...
  @Autowired
  public NoteService(
      NoteRepository notes, NoteTagRepository noteTags, NoteRevisionRepository revisions) {
    this.notes = notes;
    this.noteTags = noteTags;
    this.revisions = revisions;
  }

  public NoteService(NoteRepository notes, NoteTagRepository noteTags) {
    this(notes, noteTags, null);
  }

  public NoteService(NoteRepository notes) {
    this(notes, null, null);
  }
  ...
  public List<NoteResponse> list(User user) {
    return notes.findAllByUserAndStatusOrderByUpdatedAtDesc(user, "active").stream()
        .map(this::response)
        .toList();
  }
  ...
  public List<NoteResponse> trash(User user) {
    return notes.findAllByUserAndStatusOrderByUpdatedAtDesc(user, "trash").stream()
        .map(this::response)
        .toList();
  }

  @Transactional(readOnly = true)
  public NoteSearchResponse search(
      User user, String query, String tag, Boolean favorite, String status, Pageable pageable) {
    Page<Note> result =
        notes.findAll(NoteSpecifications.search(user, query, tag, favorite, status), pageable);
    return new NoteSearchResponse(
  ...
  @Transactional(readOnly = true)
  public NoteResponse get(User user, Long id) {
    return response(findOwned(user, id));
  }
  ...
  public NoteResponse update(User user, Long id, NoteUpdateRequest request) {
    ...
    boolean changed =
        !Objects.equals(note.getTitle(), request.title())
  ...
  public NoteResponse updateReviewStatus(User user, Long id, ReviewStatusRequest request) {
    if (!REVIEW_STATUSES.contains(request.reviewStatus())) {
      throw new com.example.studyvault.exception.InvalidReviewStatusException(
  ...
  private NoteResponse response(Note note) {
    ...
    var assigned =
        noteTags.findAllByNote(note).stream()
            .map(com.example.studyvault.entity.NoteTag::getTag)
            .map(com.example.studyvault.dto.TagResponse::from)
            .toList();
