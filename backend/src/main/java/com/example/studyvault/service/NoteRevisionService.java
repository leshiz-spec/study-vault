...
public class NoteRevisionService {
  ...
  public NoteRevisionService(
      NoteRepository notes, NoteRevisionRepository revisions, NoteService noteService) {
    this.notes = notes;
  ...
  public List<NoteRevisionResponse> list(User user, Long noteId) {
    ...
    return revisions
        .findAllByNote_IdAndNote_User_IdOrderByCreatedAtDesc(noteId, user.getId())
        .stream()
  ...
  public NoteRevisionResponse get(User user, Long noteId, Long revisionId) {
    ownedNote(user, noteId);
    return revisions
        .findByIdAndNote_IdAndNote_User_Id(revisionId, noteId, user.getId())
        .map(NoteRevisionResponse::from)
  ...
  public NoteResponse restore(User user, Long noteId, Long revisionId) {
    ...
    return noteService.update(
        user, noteId, new NoteUpdateRequest(revision.getTitle(), revision.getContent()));
  }
