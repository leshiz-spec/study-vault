...
public class NoteTagService {
  ...
  public NoteTagService(NoteRepository notes, TagService tags, NoteTagRepository noteTags) {
    this.notes = notes;
    this.tags = tags;
  ...
  public NoteResponse add(User user, Long noteId, Long tagId) {
    Note note =
        notes.findByIdAndUser(noteId, user).orElseThrow(() -> new NoteNotFoundException(noteId));
  ...
  public NoteResponse response(Note note) {
    List<TagResponse> assigned =
        noteTags.findAllByNote(note).stream().map(NoteTag::getTag).map(TagResponse::from).toList();
