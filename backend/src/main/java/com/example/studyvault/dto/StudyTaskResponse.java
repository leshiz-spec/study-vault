...
public record StudyTaskResponse(
    ...
    public static StudyTaskResponse from(StudyTask task) {
        ...
        return new StudyTaskResponse(task.getId(), task.getTitle(), task.getDueDate(), task.getStatus(),
                note == null ? null : note.getId(), note == null ? null : note.getTitle(),
    ...
    public StudyTaskResponse(Long id, String title, LocalDate dueDate, String status, Long noteId,
                             String noteTitle, OffsetDateTime createdAt, OffsetDateTime updatedAt) {
        this(id, title, dueDate, status, noteId, noteTitle,
                noteId == null ? List.of() : List.of(noteId),
                noteTitle == null ? List.of() : List.of(noteTitle), createdAt, updatedAt);
    }
}
