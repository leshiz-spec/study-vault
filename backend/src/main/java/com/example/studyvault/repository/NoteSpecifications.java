...
public final class NoteSpecifications {
  ...
  public static Specification<Note> search(
      User user, String queryText, String tag, Boolean favorite, String status) {
    return (root, query, cb) -> {
      ...
      if (tag != null && !tag.isBlank())
        predicates.add(hasOwnedTag(root, query, cb, user, tag.trim()));
      return cb.and(predicates.toArray(Predicate[]::new));
    };
  }

  private static Predicate hasOwnedTag(
      Root<Note> note,
    ...
    subquery
        ...
        .where(cb.equal(noteTag.get("note"), note), cb.equal(tag.get("user"), user), tagMatch);
