...
public class NoteController {
  ...
  private final NoteTagService noteTags;
  private final NoteFileService files;
  private final NoteSummaryService summaries;
  private final NoteRevisionService revisions;

  @Autowired
  public NoteController(
      NoteService notes,
      NoteTagService noteTags,
      NoteFileService files,
      NoteSummaryService summaries,
  ...
  public NoteController(
      ...
      NoteFileService files,
      NoteSummaryService summaries) {
    this(notes, noteTags, files, summaries, null);
  }

  public NoteController(NoteService notes, NoteTagService noteTags, NoteFileService files) {
    this(notes, noteTags, files, null, null);
  }

  public NoteController(NoteService notes, NoteTagService noteTags) {
    this(notes, noteTags, null, null, null);
  }
  ...
  public ApiResponse<NoteResponse> create(
      Authentication authentication, @Valid @RequestBody NoteCreateRequest request) {
    ...
    return ResponseEntity.ok()
        .header(
            ...
            ContentDisposition.attachment()
                .filename("studyvault-notes.zip", StandardCharsets.UTF_8)
                .build()
                .toString())
  ...
  public ApiResponse<NoteResponse> importMarkdown(
  ...
  public ApiResponse<NoteResponse> updateReviewStatus(
      Authentication authentication,
      @PathVariable Long id,
  ...
  public ApiResponse<List<NoteRevisionResponse>> revisions(
      Authentication authentication, @PathVariable Long id) {
    return ApiResponse.success(revisions.list(currentUser(authentication), id));
  }

  @GetMapping("/{id}/revisions/{revisionId}")
  public ApiResponse<NoteRevisionResponse> revision(
      Authentication authentication, @PathVariable Long id, @PathVariable Long revisionId) {
  ...
  public ApiResponse<NoteResponse> restoreRevision(
      Authentication authentication, @PathVariable Long id, @PathVariable Long revisionId) {
    return ApiResponse.success(revisions.restore(currentUser(authentication), id, revisionId));
  }

  @PostMapping("/{id}/summarize")
  public ApiResponse<NoteSummaryResponse> summarize(
      Authentication authentication, @PathVariable Long id) {
  ...
  public ApiResponse<NoteResponse> saveSummary(
      Authentication authentication,
      @PathVariable Long id,
  ...
  @PostMapping("/{id}/tags")
  public ApiResponse<NoteResponse> addTag(
      Authentication authentication,
      @PathVariable Long id,
      @Valid @RequestBody NoteTagRequest request) {
  ...
  public ApiResponse<Void> removeTag(
      Authentication authentication, @PathVariable Long id, @PathVariable Long tagId) {
    noteTags.remove(currentUser(authentication), id, tagId);
